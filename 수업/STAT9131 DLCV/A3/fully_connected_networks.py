...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = x.flatten(start_dim = 1).matmul(w)+b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.normal(0.0, weight_scale, size = (input_dim, hidden_dim),dtype=torch.float64, device='cuda')
        self.params['b1'] = torch.zeros((hidden_dim,), dtype=torch.float64, device='cuda')
        self.params['W2'] = torch.normal(0.0, weight_scale, size = (hidden_dim, num_classes),dtype=torch.float64, device='cuda')
        self.params['b2'] = torch.zeros((num_classes,), dtype=torch.float64, device='cuda')
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        h, cache1 = Linear_ReLU.forward(X,self.params['W1'], self.params['b1'])
        scores,cache2 = Linear.forward(h, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss = loss + self.reg * ( (self.params['W1'] ** 2).sum() + (self.params['W2'] ** 2).sum() )


        dh,grads['W2'], grads['b2']=Linear.backward(dout, cache2)
        grads['W2'] = grads['W2'] + self.reg * self.params['W2'] **2

        dh,grads['W1'], grads['b1']=Linear_ReLU.backward(dh, cache1)
        grads['W1'] = grads['W1'] + self.reg * self.params['W1'] **2        
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.normal(0.0, weight_scale, size = (input_dim, hidden_dims[0]),dtype=dtype, device=device)
        self.params['b1'] = torch.zeros((hidden_dims[0],), device=device,dtype=dtype,)

        for i in range(0, self.num_layers-2):
          self.params["W" + str(i+2)] =  torch.normal(0.0, weight_scale, size = (hidden_dims[i], hidden_dims[i+1]),dtype=dtype, device='cuda')
          self.params['b' + str(i+2)] = torch.zeros((hidden_dims[i+1],), device=device).type(dtype)

        self.params['W' + str(self.num_layers)] =  torch.normal(0.0, weight_scale, size = (hidden_dims[self.num_layers-2], num_classes),dtype=dtype, device='cuda')
        self.params['b' + str(self.num_layers)] = torch.zeros((num_classes,), device=device).type(dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    solver = None
    solver = Solver(model, data = data_dict,
            update_rule=sgd,
            optim_config={
              'learning_rate': 1e-2,
            },
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate'] * dw)/ (torch.sqrt(grad_squared) + config['epsilon'])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate'] * m / (torch.sqrt(v) + config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
