...
from fully_connected_networks import Linear, Linear_ReLU, ReLU, Solver, adam
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        pad2d = torch.nn.functional.pad
        pad_size = conv_param['pad']
        stride = conv_param['stride']
        N, C, H, W = x.shape
        F, _, HH, WW = w.shape
        H_out = 1 + (H + 2 * pad_size - HH) // stride
        W_out = 1 + (W + 2 * pad_size - WW) // stride
        #pad를 마지막 2개 dm(cnn에서는 width와 height)에만 주려면,
        #(1,1,1,1) 형태로 줘야 함. (width에만 주면 (1,1))
        x_ = pad2d(x, (pad_size, pad_size, pad_size, pad_size), mode='constant')
        out = torch.zeros((N, F, H_out, W_out), device = x.device, dtype= x.dtype)
        for f in range(F):
          for i in range(H_out): 
            for j in range(W_out): 
              out[:, f, i, j] = torch.sum(x_[:,:,i*stride:i*stride+HH, \
                j*stride:j*stride+WW] * w[f], axis=(1,2,3)) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros(x_.shape, dtype=x.dtype, device=x.device)
        for n in range(N): 
          for f in range(F):
            ...
            for i in range(H_out): 
              for j in range(W_out): 
                dw[f, :, :, :] += dout[n, f, i, j] *\
                x_[n, : , i*stride:i*stride+HH, j*stride:j*stride+WW]
                dx[n, :, i*stride:i*stride+HH,j*stride:j*stride+WW] += \
                 dout[n, f, i, j] * w[f, :, :, :]
        dx = dx[:, :, pad_size: H+pad_size, pad_size: W+pad_size]
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
          ...
          for c in range(C):
            for h in range(H_out):
              for w in range(W_out):
                y_start, y_end = stride * h, stride*h+pool_height
                x_start, x_end = stride * w, stride*w+pool_width
                out[n,c,h,w] = torch.max(x[n,c,y_start:y_end,x_start:x_end])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C):
            for h in range(H_out):
              for w in range(W_out):
                y_start, y_end = stride * h, stride*h+pool_height
                x_start, x_end = stride * w, stride*w+pool_width
                pool_X = x[n,c,y_start:y_end,x_start:x_end]     
                mask = (pool_X == torch.max(pool_X))    
                dx[n,c,y_start:y_end,x_start:x_end] = mask * dout[n,c,h,w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = torch.normal(0,weight_scale,(num_filters*H_out*W_out,hidden_dim),dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_relu_pool_out, cache_conv_relu_pool = Conv_ReLU_Pool.forward(X,W1,b1,conv_param,pool_param)
        out_linearRelu , cache_linearrlu = Linear_ReLU.forward(conv_relu_pool_out, W2, b2)
        scores, cache_linear = Linear.forward(out_linearRelu, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        d_hid2 , grads['W3'] , grads['b3'] = Linear.backward(dout, cache_linear)
        d_hid1, grads['W2'], grads['b2'] = Linear_ReLU.backward(d_hid2, cache_linearrlu)
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(d_hid1, cache_conv_relu_pool)
...
class DeepConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            L2reg += torch.sum(w ** 2)
            ##use already made function for it...
            ##pooling and batchnorms are options, so pit them as conditions.. if.
        ...
        # fully connected layer is done separately
        i += 1
        ...
        loss +=  L2reg
        # Backward propagation, so begin from the linear layer
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims,
                        ...
                        max_pools=[1,2,3],
                        batchnorm=False,
                        num_classes=10, weight_scale='kaiming',
                        reg=7e-4,
                        weight_initializer=None,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        fan_in = Din
        weight = torch.randn(Din, Dout, dtype=dtype, device=device) * ((gain / fan_in) ** .5)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        # Replace "pass" statement with your code
        fan_in = Din * K * K
        weight = torch.randn(Dout, Din, K, K, dtype=dtype, device=device) * ((gain / fan_in) ** .5)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            x_norm = (x-mean) /((var+eps)**(.5))
            out = gamma * x_norm + beta

            cache = (x_norm, gamma,x, mean,  var, eps,beta,running_mean, running_var)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            ################################################################
            # Replace "pass" statement with your code
            x_hat = (x-running_mean) /((running_var+eps)**(.5))
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x_norm, gamma,x, mean,  var, eps,beta,running_mean, running_var = cache
        N = dout.shape[0]
        ...
        dvar = (dx_hat * (x-mean) * (-0.5) * (var + eps) ** (-3 / 2)).sum(axis = 0)

        dmean = - dx_hat.sum(axis = 0) /((var+eps)**(.5)) + dvar * -2 * (x-mean).sum(axis = 0) / N
        
        dx = dx_hat/((var+eps)**(.5)) + dvar * 2 * (x-mean) / N +  dmean / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        # Store the results in the dx, dgamma, and dbeta variables.       #
        #                                                                 #
        # Just to note, after computing the gradient with respect to the  #
        # centered inputs, you should be able to compute gradients with   #
        # respect to the inputs (i.e., dx) in a single statement; our     #
        # implementation fits on a single 80-character line.              #
        # But, it is okay to write it in multiple lines.                  #
        ###################################################################
        # Replace "pass" statement with your code
        x_norm, gamma,x, mean,  var, eps,beta,running_mean, running_var = cache
        N = dout.shape[0]
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        m, cache = BatchNorm.forward(m, gamma, beta, bn_param)
        out = m.T.reshape(C, N, H, W).permute(1, 0, 2, 3)
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
