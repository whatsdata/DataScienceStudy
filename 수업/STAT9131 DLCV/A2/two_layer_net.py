...
def nn_forward_backward(
    ...
    real_probs = probs[range(num_train), y]
    loss = (-torch.log(real_probs)).sum()
    loss /= num_train
    ...
    grads['b1'] = dh1.sum(axis=0)/num_train
    
    grads['W1'] = X.T.matmul(dh1)/num_train
    grads['W1'] += reg * 2 * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    hidden_sizes = [ 1000,  3000]
    regularization_strengths = [1e-6,  1e-7]
    learning_rate_decays = [95e-2, 99e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
