...
def svm_loss_naive(
    ...
    # and add it to dW. (part 2) Don't forget to divide your gradient by        #
    # num_train first, if you have not done so in part 1.                       #
    #############################################################################
    # Replace "pass" statement with your code
    dW /= num_train
    dW += 2* reg *W
...
def svm_loss_vectorized(
    ...
    correct_class_scores =torch.ones_like(scores)*( scores[y, torch.arange(0, scores.shape[1])])
    margins = scores - correct_class_scores + torch.ones_like(scores)
    margins[margins<0] = 0
    margins[y, torch.arange(0, scores.shape[1])] = 0
    ...
    margins[y, torch.arange(0, scores.shape[1])] = -1 * torch.sum(margins, axis=0)
    dW= torch.mm(margins,X).T
    dW /= num_train
    dW += 2* reg *W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
    ...
    regularization_strengths= [0.001,0.01,100]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
