...
def hello_pytorch_autograd_and_nn():
    """
...
def three_layer_convnet(x, params):
    ...
    # 1. Conv layer (with bias) with given filters (32 5x5 filters from params), #
    #        with zero-padding of 2                                              #
    # 2. ReLU                                                                    #
    # 3. Conv layer (with bias) with given filters (16 3x3 filters from params), #
    #        with zero-padding of 1                                              #
    # 4. ReLU                                                                    #
    # 5. Fully-connected layer (with bias) to compute scores for 10 classes      #
    # Hint: F.linear, F.conv2d, F.relu, flatten (implemented above)              #
    ##############################################################################
    # Replace "pass" statement with your code
    c1 = F.conv2d(x, conv_w1, conv_b1,  padding=2)
    r1 = F.relu(c1)
    c2 = F.conv2d(r1, conv_w2, conv_b2, padding=1)
    r2 = F.relu(c2)
    scores = F.linear(flatten(r2), fc_w, fc_b)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return scores
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
    ...
    dim1 = 1+(H-kernel_size_1 + 2*2)
    dim2 = 1+(dim1-kernel_size_2 +1*2)
    fc_w = nn.init.kaiming_normal_(torch.empty((num_classes,channel_2*dim2*dim2), dtype=dtype, device=device))
...
class ThreeLayerConvNet(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, num_classes):
        ...
        # since the dimension of image does not shrink, it is just *H*W
    ...
    def forward(self, x):
        ...
        # Hint: F.relu, flatten (implemented at the start of part II)              #                                     
        ############################################################################
        # Replace "pass" statement with your code
...
def initialize_three_layer_conv_part4():
    ...
    # Hint: nn.Sequential, nn.Flatten
    # Hint: you might also want to use OrderedDict to give a specific name to each
    #             module; see the example of Two-Layer Network in ipynb
    ####################################################################################
    # Replace "pass" statement with your code
    model = nn.Sequential(
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Hint: you might still need `flatten`.                                    #
        ############################################################################
        # Replace "pass" statement with your code
        out = self.cnn(x)
        # print(out.shape)
