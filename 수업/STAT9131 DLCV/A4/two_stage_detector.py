...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        _cnn = torchvision.models.regnet_x_400mf(pretrained=True)
        ...
        self.backbone = torchvision.models.feature_extraction.create_feature_extractor(
            _cnn,
            return_nodes={
        ...
        #                                                                    #
        # HINT: You may iterate over `dummy_out_shapes` defined above to     #
        # decide the input/output channels of these layers. For convenience, #
        # You may name layers in the format of "c#_lateral" and "c#_output"  #
        # to iterate over them in the forward implementation.                #
        ######################################################################
        ...
        self.fpn_params['cv5'] = nn.Conv2d(dummy_out['c5'].shape[1], self.out_channels, 1)
        self.fpn_params['cv4'] = nn.Conv2d(dummy_out['c4'].shape[1], self.out_channels, 1)
        self.fpn_params['cv3'] = nn.Conv2d(dummy_out['c3'].shape[1], self.out_channels, 1)

        self.fpn_params['cv_o5'] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params['cv_o4'] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params['cv_o3'] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def fpn_strides(self):
        ...
        to implement your logic in Faster R-CNN.
        """
    ...
    def forward(self, images: torch.Tensor):
        ...
        # HINT: Use `F.interpolate` with a proper `scale_factor` to upsample #
        # FPN features. Use nearest neighbor upsampling for simplicity.      #
        ######################################################################

        # Replace "pass" statement with your code
        out5 = self.fpn_params['cv5'](backbone_feats["c5"])
        out4 = torch.add(F.interpolate(out5, scale_factor=2), self.fpn_params['cv4'](backbone_feats["c4"]))
        out3 = torch.add(F.interpolate(out4, scale_factor=2), self.fpn_params['cv3'](backbone_feats["c3"]))

        fpn_feats["p5"] = self.fpn_params['cv_o5'](out5)
        fpn_feats["p4"] = self.fpn_params['cv_o4'](out4)
        fpn_feats["p3"] = self.fpn_params['cv_o3'](out3) 
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        stem_rpn = []
        rpn_cv1=nn.Conv2d(in_channels,stem_channels[0],3,1,1)
        nn.init.normal_(rpn_cv1.weight,mean=0.0, std=0.01)
        stem_rpn.append(rpn_cv1)
        stem_rpn.append(nn.ReLU())
        for i in range(len(stem_channels)-1):
            cv=nn.Conv2d(stem_channels[i],stem_channels[i+1],3,1,1)
            nn.init.normal_(cv.weight,mean=0.0, std=0.01)
            stem_rpn.append(cv)
            stem_rpn.append(nn.ReLU())
        ...
        # Apply the same initialization as above.
        #
        # Objectness is obtained by applying sigmoid to its logits. However,
        # DO NOT create a sigmoid module here. PyTorch loss functions have
        ...
        # Replace "pass" statement with your code
        cv_cls=nn.Conv2d(stem_channels[-1],1*num_anchors,1)
        nn.init.normal_(cv_cls.weight,mean=0.0, std=0.01)
        self.pred_obj=cv_cls

        cv_reg=nn.Conv2d(stem_channels[-1],4*num_anchors,1)
        nn.init.normal_(cv_reg.weight,mean=0.0, std=0.01)
        self.pred_box=cv_reg
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # HINT: You might want contiguous() after permute() to avoid any error.
        ######################################################################
        ...
        for key, feat in feats_per_fpn_level.items():
          ...
          B, C, H, W = boxreg_deltas[key].shape
          boxreg_deltas[key] = boxreg_deltas[key].reshape(B, 4, H*W*self.num_anchors).permute(0,2, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Be aware that the indices go from top-to-bottom and left-to-right. #
        ######################################################################
        # Replace "pass" statement with your code
        # Replace "pass" statement with your code
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            #
            # First, calculate resulting width and height of the anchor box as
            # per `stride_scale` and `aspect_ratios` definitions, where the
            # formulation is described above. Then, shift the locations to get
            # top-left and bottom-right co-ordinates.
            #
            # Append generated boxes to `anchor_boxes` list above.
            ##################################################################
            # Replace "pass" statement with your code
            xc=locations[:,0].reshape((len(locations),-1))
            yc=locations[:,1].reshape((len(locations),-1))
            area=torch.tensor((stride_scale*level_stride)**2)
            w=torch.sqrt(area / aspect_ratio)
            h=area / w
            cetner = torch.hstack((locations, locations))
            edges = torch.tensor((-w/2, -h/2, w/2, h/2), device=locations.device)
            anchor_boxes.append(cetner + edges)
            ##################################################################
            #                           END OF YOUR CODE                     #
            ##################################################################
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    xx1 = torch.maximum(boxes1[:, 0].unsqueeze(1), boxes2[:, 0]) 
    yy1 = torch.maximum(boxes1[:, 1].unsqueeze(1), boxes2[:, 1])
    xx2 = torch.minimum(boxes1[:, 2].unsqueeze(1), boxes2[:, 2])
    yy2 = torch.minimum(boxes1[:, 3].unsqueeze(1), boxes2[:, 3])
    invalid_idx = torch.logical_or(xx1 > xx2, yy1 > yy2) 
    inter_boxes = torch.stack((xx1, yy1, xx2, yy2), dim=-1).reshape(-1,4) 
    inter_areas = ((inter_boxes[:,2]-inter_boxes[:,0])*(inter_boxes[:,3]-inter_boxes[:,1])).reshape(M, N)
...
def rcnn_get_deltas_from_anchors(
    ...
    deltas will become GT targets for box regression. The deltas are in
    `(dx, dy, dw, dh)` format that represent offsets to anchor centers
...
def rcnn_apply_deltas_to_anchors(
    ...
    px = (anchors[:, 0] + anchors[:, 2]) / 2 ; py = (anchors[:, 1] + anchors[:, 3]) / 2
    pw = anchors[:, 2] - anchors[:, 0]       ; ph = anchors[:, 3] - anchors[:, 1]
    bx = px + pw * deltas[:, 0]              ;  by = py + ph * deltas[:, 1]
    bw = pw * torch.exp(deltas[:, 2])        ; bh = ph * torch.exp(deltas[:, 3])
...
class RPN(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            nms_thresh: IoU threshold for NMS - this is used
    ...
    def forward(
        ...
        # appropriate functions with proper arguments. Recall that, before
        # generating anchors using `generate_fpn_anchors`, you may run
        # `get_fpn_location_coords` to get `locations_per_fpn_level`.
        ######################################################################
        ...
        for level_name, feat in feats_per_fpn_level.items():
            shape[level_name]=feat.shape
            
        dtype = feats_per_fpn_level["p3"]
        device = feats_per_fpn_level["p3"].device
        locations=get_fpn_location_coords(shape,strides_per_fpn_level,
                                          dtype=dtype.dtype,device=device)
        ...
        ######################################################################
        # TODO: Match the generated anchors with provided GT boxes. This
        # function is not batched so you may use a for-loop for each gt box.
        # Hint: rcnn_match_anchors_to_gt
        ######################################################################
        ...
        for i in range(num_images):
            matched_gt_boxes.append(rcnn_match_anchors_to_gt(anchor_boxes,
                                    gt_boxes[i],self.anchor_iou_thresholds))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        if self.training:
            ...
            #      there aren't enough foreground anchors. You may collect
            #      all indices and use them to sample boxes and logits from
            #      above for next steps.
            #
            #   2. Compute GT targets for box regression from `anchor_boxes`
            #      and `matched_gt_boxes` on the foreground.
            #      HINT: `rcnn_get_deltas_from_anchors`. Use the last column of
            #      `matched_gt_boxes` to filter out background anchor masks.
            #
            #   3. Calculate objectness and box reg losses per sampled anchor.
            #      Remember to set box loss for "background" anchors to 0.
            #      HINT: Use the last column of `matched_gt_boxes` for computing
            #      objectness. Set reduction="none" when computing losses,
            #      as we average them later.
            ##################################################################
            # Feel free to delete this line: (but keep variable names same)
            loss_obj, loss_box = None, None
            fg_idx, bg_idx = sample_rpn_training(matched_gt_boxes, 
              self.batch_size_per_image * num_images, 0.5)
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                #      HINT: `rcnn_apply_deltas_to_anchors`
                ...
                #      (actually logits; with or without sigmoid doesn't matter)
                #      to the dicts above (`proposals_per_fpn_level_per_image`
                #      and `scores_per_fpn_level_per_image`).
                #
                # NOTE: We ask you to implement `nms` later.
                # However, your NMS implementation may be slow for training.
                # Hence, use `torchvision.ops.nms` to speed up training here.
                ##############################################################
                # Replace "pass" statement with your code
                ...
                boxes[:,2] = boxes[:,2].clamp(max=image_size[0])
                boxes[:,3] = boxes[:,3].clamp(max=image_size[1])


                if self.pre_nms_topk<level_obj_logits.shape[0]:
                    level_obj_logits, order = torch.topk(level_obj_logits, self.pre_nms_topk)
                else:
                    level_obj_logits, order = torch.sort(level_obj_logits, descending=True)

                boxes = boxes[order]
                keep = torchvision.ops.nms(boxes,level_obj_logits,self.nms_thresh)
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    for i in range(n):
        ...
        x1 = torch.clamp(boxes[:, 0], min=ix1) 
        y1 = torch.clamp(boxes[:, 1], min=iy1) 
        x2 = torch.clamp(boxes[:, 2], max=ix2) 
        y2 = torch.clamp(boxes[:, 3], max=iy2) 
...
class FasterRCNN(nn.Module):
    ...
    Unlike Faster R-CNN, we will skip class-specific box regression at the second stage.
    """
    ...
    def __init__(
        ...
        # `RPNPredictionNetwork`. use the same stride, padding, and
        # weight initialization as previous TODOs.
        #
        ...
        cv1=nn.Conv2d(in_channels,stem_channels[0],3,1,1)
        nn.init.normal_(cv1.weight,mean=0.0, std=0.01)
        cls_pred.append(cv1)
        ...
        # shape from `nn.Flatten` layer. Use the same weight initialization as
        # above.
        ######################################################################
        # Replace "pass" statement with your code
    ...
    def forward(
        ...
        for level_name in feats_per_fpn_level.keys():
            ...
            roi_feats=torchvision.ops.roi_align(level_feats,
                level_props,self.roi_size,spatial_scale=1/level_stride)
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
        ...
        ######################################################################
        # TODO: Match the RPN proposals with provided GT boxes and append to
        # `matched_gt_boxes`. Use `rcnn_match_anchors_to_gt` with IoU threshold
        # such that IoU > `self.iou_thresh` is foreground, otherwise background.
        # As in `__init__`, We use `self.iou_thresh = 0.5` for this assignment.
        ...
        # HINT: You may copy some of your implementation in RPN and modify it.
        # Set reduction="mean" when computing the classification loss.
        ######################################################################
         # Feel free to delete this line: (but keep variable names same)
        loss_cls = None
    ...
    def inference(
        ...
        ######################################################################
        # TODO: Faster R-CNN inference, perform the following steps in order:
        ...
        #      Update `pred_boxes`, `pred_scores`, and `pred_classes`.
        #
        ...
        # NOTE: Complete your NMS implementation before testing this.
        ######################################################################
        pred_scores, pred_classes = None, None
        # Replace "pass" statement with your code
        
        level=F.softmax(pred_cls_logits, dim=1)
        pred_scores,pred_classes=torch.max(level,dim=1)
       
        pred_classes-=1
        keep = (pred_scores > test_score_thresh) & (pred_classes > -1)
