...
def load_CIFAR(path='./'):
    ...
    cifar10_train = datasets.CIFAR10(path, train=True, download=True, transform=transform)
    loader_train = DataLoader(cifar10_train, batch_size=64,
                              sampler=sampler.SubsetRandomSampler(range(NUM_TRAIN)))

    cifar10_val = datasets.CIFAR10(path, train=True, download=True, transform=transform)
    loader_val = DataLoader(cifar10_val, batch_size=64,
                            sampler=sampler.SubsetRandomSampler(range(NUM_TRAIN, 50000)))

    cifar10_test = datasets.CIFAR10(path, train=False, download=True, transform=transform)
...
class VOC2007DetectionTiny(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        Args:
            download: Whether to download PASCAL VOC 2007 to `dataset_dir`.
                If `tmp_dir` is not None, then extract the dataset there;
                otherwise, extract the dataset in `dataset_dir` as well.
        ...
        if download:
            self._attempt_download(dataset_dir, tmp_dir)
    ...
    @staticmethod
    def _attempt_download(dataset_dir: str, tmp_dir: str):
        """
        Try to download VOC dataset to `dataset_dir` and extract to `tmp_dir`.
        """
        if tmp_dir is None:
            tmp_dir = dataset_dir
        if not os.path.isdir(os.path.join(tmp_dir, "VOCdevkit")):
            os.makedirs(tmp_dir, exist_ok=True)
            os.makedirs(dataset_dir, exist_ok=True)

            # fmt: off
            if not os.path.isfile(os.path.join(dataset_dir, "VOCtrainval_06-Nov-2007.tar")):
                import wget
                wget.download(
...
def inference_with_detector(
    ...
    for iter_num, test_batch in enumerate(test_loader):
        ...
        if output_dir is not None:
            ...
            common.utils.detection_visualizer(
