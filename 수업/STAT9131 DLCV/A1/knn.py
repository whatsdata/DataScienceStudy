...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i,j] = torch.sqrt(torch.sum(torch.square(x_train[i,:] - x_test[j,:]) ))
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      dists[i,:] = torch.sqrt(torch.sum(torch.square(torch.flatten(x_test-x_train[i,:],start_dim=1)),axis=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    train = torch.sum(torch.square(torch.flatten(x_train, start_dim=1)),axis=1)
    test = torch.sum(torch.square(torch.flatten(x_test, start_dim=1)),axis=1)
    mul = torch.mm(torch.flatten(x_train, start_dim=1),torch.flatten(x_test,start_dim=1).T)
    dists = torch.sqrt(test -2*mul + train.reshape(-1,1))
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      index = torch.argsort(dists[:,i])[:k]
      k_nearest_y = y_train[index]
      y_pred[i] = torch.min(torch.argmax(torch.bincount(k_nearest_y)))
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        for i in range(num_test):
           index = torch.argsort(dists[:,i])[:k]
           k_nearest_y = self.y_train[index]
           y_test_pred[i] = torch.min(torch.argmax(torch.bincount(k_nearest_y)))        
       ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        x_train_cv = torch.cat([x_train_folds[j] for j in range(num_folds) if i != j])
        y_train_cv = torch.cat([y_train_folds[j] for j in range(num_folds) if i != j])
        x_validation = x_train_folds[i]
        y_validation = y_train_folds[i]
        knn = KnnClassifier(x_train_cv, y_train_cv)
        accuracies.append(knn.check_accuracy(x_validation, y_validation, k=k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    best_k = sorted(k_to_accuracies.items())[
        (torch.argmax(torch.tensor(means))).item()][0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
