...
def reparametrize(mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    epsilon = torch.randn(mu.shape).to(mu)
    z = mu + epsilon * (torch.exp(logvar))**(1/2)
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # Replace "pass" statement with your code
    N,Z = mu.shape
    loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum')/N
    KL = -0.5 * torch.sum(1+logvar - mu**2 - torch.exp(logvar)) / N
    loss = loss + KL
